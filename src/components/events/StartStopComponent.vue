<template>
  <div class="flex gap-2">
    <button
      v-if="event.startTime != null"
      @click="$router.push({ path: `/activeevent/${eventId}` })"
      class="p-4 h-full text-white text-xl rounded-lg shadow-lg hover:bg-opacity-80 hover:duration-200 duration-200 text-center flex justify-center bg-primary-orange"
    >
      {{ event.stopTime == null ? 'Live event statistics' : 'Event log' }}
    </button>
    <button
      v-if="event.stopTime == null"
      @click="startStop()"
      :class="
